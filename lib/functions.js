...
module.exports.getBookcoverUrl = (req, res) => {
    ...
    switch(urlString.pathname){
        case '/getBookCover':
            const query = new URLSearchParams(urlString.query);
            if(query.has('bookTitle') || query.has('authorName')){
                let bookTitle = query.get('bookTitle')
                let authorName = query.get('authorName')
                //let queryString = `${bookTitle.replace(' ', '+')} ${authorName.replace(' ', '+')}`
                //making request to google to get book's goodreads page
                axios.get(`https://www.google.com/search?q=${bookTitle}+${authorName}+goodreads&sourceid=chrome&ie=UTF-8`)
                .then((response) => {
